...
/* The initial stack pointer, points at the top of RAM
 *  FIXME Should not be hard-coded.
 * When its is defined in the linker script, it is not constant
 * and therefore does not work for the vectors array.*/
const unsigned long initStack112k = 0x2001c000;

/**
  *@brief Interrupt vector table in FLASH, just used for startup */

__attribute__ ((section(".isr_vectorsflash")))
void (* const g_pfnVectorsStartup[])(void) =
{
		(intfunc)((unsigned long)0x2001c000), // The initial stack pointer during startup
		Reset_Handler,             			 // The reset handler during startup
};

// After startup, the vector table location is configured to be in RAM, so we have
// to put the full table there.
__attribute__ ((section(".isr_vectorsram")))
void (* const g_pfnVectors[])(void) =
{
  /*----------Core Exceptions------------------------------------------------ */
  (intfunc)((unsigned long)0x2001c000), /* The initial stack pointer FIXME Should not be hard-coded. When its is defiend in the linker script, it is not constant and therefore does not work for the array.*/
...
void Default_Reset_Handler(void)
{
  ...
  // This function sets up the clocks and similar low level stuff.
  SystemInit();
