...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_simulink2CRRCSim_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_simulink2CRRCSim_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_simulink2CRRCSim_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "simulink2CRRCSim") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_simulink2CRRCSim_autoinheritance_info(nlhs,plhs,3,newRhs);
  }
...
{
  extern unsigned int sf_simulink2CRRCSim_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "simulink2CRRCSim") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_simulink2CRRCSim_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_simulink2CRRCSim_method_dispatcher(SimStruct *S, unsigned
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"simulink2CRRCSim")) {
    return(sf_simulink2CRRCSim_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void simulink2CRRCSim_terminator(void);
...
extern void simulink2CRRCSim_initializer(void);
extern void simulink2CRRCSim_register_exported_symbols(SimStruct* S);
extern void simulink2CRRCSim_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  simulink2CRRCSim_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      simulink2CRRCSim_debug_initialize();
    }

    simulink2CRRCSim_initializer();
  }
